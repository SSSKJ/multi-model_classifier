...
def preprocessor(file_name, save_name):

    df = pd.DataFrame(data = pd.read_csv(f'../data/{file_name}.csv', sep = ';', header = 0))
    ...
    df.to_csv(f'../processed_data/{save_name}.csv', index = False)
